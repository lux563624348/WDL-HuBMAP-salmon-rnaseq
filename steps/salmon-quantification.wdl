...
task TrimReads {
    ...
    command {
        ...
        /opt/trim_reads.py ~{assay} ~{adj_fastq_dir} ~{sep(" ", orig_fastq_dirs)} --threads ~{threads}
    }
...
task Salmon {
    ...
    command {
        ...
        /opt/salmon_wrapper.py ~{assay} ~{trimmed_fastq_dir} ~{sep(" ", orig_fastq_dirs)} --threads ~{threads} ~{if defined(expected_cell_count) then "--expected-cell-count " + expected_cell_count else ""} ~{if defined(keep_all_barcodes) && keep_all_barcodes then "--keep-all-barcodes" else ""}
    }
...
task SalmonMouse {
    ...
    command {
        ...
        /opt/salmon_mouse_wrapper.py ~{assay} ~{trimmed_fastq_dir} ~{sep(" ", orig_fastq_dirs)} --threads ~{threads} ~{if defined(expected_cell_count) then "--expected-cell-count " + expected_cell_count else ""} ~{if defined(keep_all_barcodes) && keep_all_barcodes then "--keep-all-barcodes" else ""}
    }
